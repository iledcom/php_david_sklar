...
/*
Если для столбца dish_name таблицы передано наименование блюда Fried Bean Curd, то
запрос SQL из примера 8.51 будет обработан как обычно: цена 1 доллар будет установлена только
на указанное блюдо. Но если в значении элемента массива $_POST['dish_name'] окажется знак
%, то цена 1 доллар будет установлена на все блюда в таблице! Этот недостаток можно устранить
с помощью метода quote() и функции strtr(). В примере 8.52 демонстрируется правильный
способ обновления цены на указанное блюдо.
*/

Пример 8.52. Правильное применение метода quote() и функции strtr() в команде UPDATE
// Сначала заключить переданное на обработку значение в кавычки
...
/*
В примере 8.53 демонстрируется законченная программа для составления формы и извлечения
записей из базы данных. Эта программа предоставляет форму поиска и выводит на экран HTML-
таблицу, состоящую из всех строк в таблице dishes базы данных, удовлетворяющих заданным
критериям поиска. Как и программа из примера 8.28, данная программа основана на вспомогательном 
...
/*
Исходный код из примера 8.53 во многом похож на исходный код из примера 8.28. В нем применяется 
стандартная структура для отображения, проверки достоверности и обработки формы с глобальным 
кодом для подключения и взаимодействия с базой данных в функции process_form(). А
в функции show_form() отображается HTML-разметка формы, определенная в файле retrieve-form.php.
Содержимое этого файла приведено в примере 8.54.
*/
...
<?php
/*
Исходный код из примера 8.53 отличается дополнительной строкой при подключении к базе
данных, где вызывается функция setAttribute(), устанавливающая режим извлечения записей.
Этот режим очень важен для извлечения информации из базы данных в функции process_form().
В функции process_form() составляется запрос SQL с командой SELECT, который затем
отправляется в базу данных с помощью метода execute(), а полученные результаты извлекаются
с помощью fetchAll() и выводятся на экран в виде HTML-таблицы. В предложении WHERE,
применяемом в команде SELECT, указывается до четырех логических условий. Два первых условия
определяют минимальную и максимальную цену на блюдо. А поскольку они всегда присутствуют в
запросе, то получают замещающие знаки в переменной $sql, где хранится запрос SQL.
Далее следует наименование блюда. И хотя это необязательно, но если оно все же передается
на обработку, то включается в запрос. Тем не менее замещающий знак не вполне подходит для
столбца dish_name, поскольку данные из переданной формы могут содержать подстановочные
символы SQL. Вместо этого с помощью метода quote() и функции strtr() подготавливается
прошедший “санобработку” вариант наименования блюда, который затем вводится непосредственно
в предложение WHERE.
И последним в предложении WHERE указывается столбец is_spicy. Если на обработку передан
вариант выбора yes, то в запрос SQL вводится логическое условие AND is_spicy = 1, чтобы из
таблицы базы данных были извлечены только блюда со специями. Если же на обработку передан
вариант выбора no, то в запрос SQL вводится логическое условие AND is_spicy = 0, чтобы в
таблице базы данных были найдены только блюда без специй. А если на обработку передан вариант
выбора either, то указывать столбец is_spicy в запросе не нужно, поскольку строки должны
быть извлечены из таблицы независимо от того, приправлены ли они специями.
Как только запрос SQL будет полностью составлен в переменной $sql, он подготавливается с
помощью функции prepare() и направляется программе базы данных с помощью метода execute(). В
качестве второго аргумента метода execute() указывается массив, содержащий значения минималь-
ной и максимальной цены, чтобы подставить их вместо замещающих знаков. Массив строк таблицы,
возвращаемый функцией fetchAll(), сохраняется в переменной $dishes.
И, наконец, в функции process_form() выводятся результаты обработки запроса SQL. Ес-
ли данные в переменной $dishes отсутствуют, то на экран выводится сообщение "No dishes
matched" (Совпавшие блюда отсутствуют). В противном случае содержимое переменной $dishes
выводится в цикле foreach() в виде строк HTML-таблицы с каждым блюдом в отдельности,
причем для надлежащего форматирования цены на блюдо вызывается функция printf(), а для
